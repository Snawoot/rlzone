...
func (z *RatelimitZone[K, V]) AllowN(key K, n uint64) bool {
	...
	if val+float64(n) > float64(z.limit) {
		return false
	}

	z.shiftMaps(reqPrevWndStart, reqCurrWndStart)

	z.incCounter(key, reqCurrWndStart, V(n))
...
func (z *RatelimitZone[K, V]) incCounter(key K, wndStart time.Time, increment V) {
	if m := z.getWndMap(wndStart, false); m != nil {
		m[key] += increment
	}
}
